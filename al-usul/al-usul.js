...
function renderTableForRawi(rawiName) {
   ...
   // ============================
    ...
    // 8.1. حساب عدد الصفوف المدمجة لكل قيمة في case1
    const case1Groups = [];
    let i = 0;
    ...
    while (i < rows.length) {
        let j = i + 1;
        while (j < rows.length && rows[j].case1 === rows[i].case1) {
            j++;
        }
        case1Groups.push({
            value: rows[i].case1,
    ...
    rows.forEach((row, index) => {
        ...
        // عرض الصف فقط إذا كان هناك حكم أو كان تنبيهاً
        if (rule !== '-' || isAlert) {
            hasData = true;
            ...
            if (group && index === group.start) {
                bodyHtml += `<td class="col-case1" rowspan="${group.count}">${isAlert ? '📌 ' : ''}${row.case1 || ''}</td>`;
            } else if (group && index > group.start) {
                bodyHtml += `<td class="col-case1" style="display: none;"></td>`;
            } else {
                bodyHtml += `<td class="col-case1">${isAlert ? '📌 ' : ''}${row.case1 || ''}</td>`;
            }
            
            // عمود case2
            if (hasCase2) {
                bodyHtml += `<td class="col-case2">${isAlert ? row.case2 || '' : row.case2 || ''}</td>`;
            }
            ...
            // عمود الحكم
            bodyHtml += `<td class="col-rule rule-${colorClass}">${isAlert ? '' : rule}</td>`;
            bodyHtml += '</tr>';
        }
    });
