...
function applyColors(text) {
    ...
    // إذا كان النص يحتوي بالفعل على <span>، نتركه كما هو
    if (text.includes('<span')) {
        return text;
    }
    
    // خريطة الألوان للكلمات المفتاحية
    const colorMap = {
        'إثبات': 'c1',
        'اثبات': 'c1',
        'سكت': 'c2',
    ...
    Object.keys(colorMap).forEach(key => {
        if (result.includes(key)) {
            ...
            result = result.replace(regex, `<span class="${colorMap[key]}">${key}</span>`);
        }
    });
...
// ============================
// دالة نسخ الجدول مع الحفاظ على الألوان
// ============================
function copyTable() {
