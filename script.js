...
// ==================== دعم التفسير على الهواتف (في القائمة المنبثقة) ====================

let tafsirCollapsedMobile = false;
...
// ربط التفسير بالآيات للهواتف
function bindTafsirToAyahsMobile() {
    const ayahs = document.querySelectorAll('.ayah-wrapper');
    ayahs.forEach(ayah => {
        ayah.removeEventListener('click', handleAyahClickMobile);
        ayah.removeEventListener('touchstart', handleAyahTouchMobile);
        ayah.addEventListener('touchstart', handleAyahTouchMobile);
...
function handleAyahTouchMobile(e) {
    e.preventDefault();
    e.stopPropagation();
    const sura = parseInt(this.getAttribute('data-sura'));
    const ayahNum = parseInt(this.getAttribute('data-ayah'));
...
function toggleTafsirMobile() {
    ...
    if (tafsirBody) {
        tafsirCollapsedMobile = !tafsirCollapsedMobile;
        if (tafsirCollapsedMobile) {
            tafsirBody.classList.add('collapsed');
            if (toggleBtn) toggleBtn.innerHTML = '▼';
        } else {
            tafsirBody.classList.remove('collapsed');
...
// تحديث عند تغيير الصفحة للهواتف
...
console.log('✅ تم تفعيل نظام التفسير للهواتف');