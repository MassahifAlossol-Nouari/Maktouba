...
function updateAltBottomBar() {
    ...
    let results = [];
    let foundEntries = new Set(); // لمنع التكرار
    ...
    currentPageAyahs.forEach(w => {
        ...
        // الحصول على البدائل من pageData
        const alternatives = pageData[ayah];
        if (!alternatives) return;
        
        // التأكد من أن البدائل مصفوفة
        const altArray = Array.isArray(alternatives) ? alternatives : [alternatives];
        
        // إنشاء مفتاح فريد للآية
        const entryKey = `${currentPage}-${ayah}`;
        
        // إذا لم تتم إضافة هذه الآية من قبل
    ...
    // تجميع البدائل حسب الآية
    const groupedResults = {};
    results.forEach(r => {
        if (!groupedResults[r.ayah]) {
            groupedResults[r.ayah] = [];
        }
        groupedResults[r.ayah].push(r.alt);
    });
    
    // عرض النتائج المجمعة
    Object.keys(groupedResults).forEach(ayah => {
        const alts = groupedResults[ayah];
        const div = document.createElement('div');
        ...
        let altDisplay = '';
        if (alts.length === 1) {
            altDisplay = `أوجه= 1| ${alts[0]}`;
        } else {
            altDisplay = `أوجه= ${alts.length}| ${alts.join(' + ')}`;
        }
        
        div.innerHTML = `
            <span class="alt-text">${altDisplay}</span>
            <span class="ref">(${ayah})</span>
        `;
        div.onclick = () => {
            const target = document.querySelector(`.ayah-wrapper[data-ayah="${ayah}"]`);
            if (target) {
