...
// ========================================
// نظام القراءات البديلة (يبحث بالصفحة والآية)
// ========================================
...
function loadQiraatForMushaf(mushafFile) {
    let rawiName = mushafFile.replace('mushaf_', '').replace(/[0-9].js$/, '').replace('.js', '');
    rawiName = rawiName.replace(/[0-9]+$/, '');
    currentMushaf = rawiName;
    ...
    fetch(`Data/qiraat/${rawiName}.json`)
        ...
        .then(d => {
            ...
            console.log('📦 بيانات القراءات:', altData);
            updateAltBottomBar();
        })
        .catch(() => {
            console.warn(`⚠️ لا توجد قراءات بديلة للراوي: ${rawiName}`);
            altData = {};
            updateAltBottomBar();
        });
}

// تحديث الشريط السفلي (يبحث بالصفحة والآية)
function updateAltBottomBar() {
    ...
    // نأخذ فقط الآيات في الصفحة الحالية
    let currentPageAyahs = [];
    const singlePage = document.getElementById('singlePage');
    if (singlePage) {
        currentPageAyahs = singlePage.querySelectorAll('.ayah-wrapper');
    } else {
        const rightPage = document.getElementById('rightPage');
        if (rightPage) {
            currentPageAyahs = rightPage.querySelectorAll('.ayah-wrapper');
        }
    }
    
    if (currentPageAyahs.length === 0) {
        currentPageAyahs = document.querySelectorAll('.ayah-wrapper');
    }
    
    console.log(`📝 عدد الآيات في الصفحة: ${currentPageAyahs.length}`);
    
    // الحصول على بيانات الصفحة من altData
    ...
    console.log('📦 بيانات الصفحة:', pageData);
    
    currentPageAyahs.forEach(w => {
        const sura = w.dataset.sura;
        const ayah = w.dataset.ayah;
        if (!sura || !ayah) return;
        
        // البحث في altData باستخدام الصفحة والآية
        const item = pageData[ayah];
        if (!item) return;
        
        const textEl = w.querySelector('.ayah-text');
        const plainText = textEl ? textEl.textContent : '';
        
        console.log(`🔍 البحث في الآية ${ayah}: "${plainText}" عن "${item.word}"`);
        
        if (plainText.includes(item.word)) {
            results.push({
                sura: sura,
                ayah: ayah,
                alt: item.alt,
                altParts: item.alt.includes('+') ? item.alt.split('+').map(s => s.trim()) : [item.alt]
            });
            console.log(`✅ تم العثور على: "${item.word}" في الآية ${ayah}`);
        }
    });
    
    if (results.length === 0) {
        console.log('📭 لا توجد قراءات بديلة في هذه الصفحة');
        bar.style.display = 'none';
        return;
    }
    ...
    results.forEach(r => {
        ...
        // بناء عرض الأوجه
        let altDisplay = '';
        if (r.altParts.length === 1) {
            altDisplay = `أوجه= 1| ${r.altParts[0]}`;
        } else {
            altDisplay = `أوجه= ${r.altParts.length}| ${r.altParts.join(' + ')}`;
        }
        
        div.innerHTML = `
            <span class="alt-text">${altDisplay}</span>
            <span class="ref">(${r.ayah})</span>
        `;
        div.onclick = () => {
            const target = document.querySelector(`.ayah-wrapper[data-ayah="${r.ayah}"]`);
            if (target) {
...
// دالة إخفاء/إظهار الشريط (ينزلق إلى اليمين)
...
console.log('✅ نظام القراءات البديلة جاهز (يبحث بالصفحة والآية)');
