...
function loadQiraatForMushaf(mushafFile) {
    ...
    // التحقق من وجود ملف JSON لهذا الراوي
    ...
    // تحميل الملف الخاص بالراوي
    fetch(`Data/qiraat/${jsonFile}.json`)
        .then(r => {
            if (!r.ok) {
                throw new Error(`ملف ${jsonFile}.json غير موجود`);
            }
            return r.json();
        })
        .then(d => {
            ...
            console.log(`✅ تم تحميل قراءات الراوي: ${rawiName} (${jsonFile}.json)`);
    ...
    currentPageAyahs.forEach(w => {
        ...
        if (!foundEntries.has(entryKey)) {
            ...
            altArray.forEach(alt => {
                if (alt && alt.trim() !== '') {
                    results.push({ ayah: ayah, alt: alt });
                }
            });
        }
    });
...
function closeAltBar() {
    ...
    if (typeof altBarTimeout !== 'undefined' && altBarTimeout) {
        clearTimeout(altBarTimeout);
        altBarTimeout = null;
    }
}
