...
function findSymbolInText(fullText, element, symbolsData) {
    ...
    for (let key of sortedKeys) {
        ...
        // الطريقة 1: البحث بتطبيع المسافات
        if (normalizedText.indexOf(normalizedSymbol) !== -1) {
            symbolFound = true;
        }
        
        // الطريقة 2: البحث في النص الأصلي
        if (!symbolFound && rawText.indexOf(symbol) !== -1) {
            symbolFound = true;
        }
        
        // الطريقة 3: إزالة جميع المسافات
        if (!symbolFound) {
            const textNoSpaces = rawText.replace(/\s/g, '');
            const symbolNoSpaces = symbol.replace(/\s/g, '');
            if (textNoSpaces.indexOf(symbolNoSpaces) !== -1) {
                symbolFound = true;
            }
        }
