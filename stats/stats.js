...
function displayTable(data) {
    ...
    for (let i = 0; i < filteredData.length; i++) {
        ...
        wordCell.onclick = function(e) {
            ...
            console.log(`النقر على الكلمة: page=${page}, sura=${sura}, ayah=${ayah}, mushaf=${currentMushafFile}`);
            
            if (!currentMushafFile) {
                alert('لم يتم تحديد المصحف الحالي');
                return;
            }
...
async function loadFileFromPath(fileName) {
    currentMushafFile = fileName;  // <--- أضف هذا السطر في بداية الدالة
    try {
...
async function loadFileFromDevice(file) {
    currentMushafFile = file.name;  // <--- أضف هذا السطر في بداية الدالة
