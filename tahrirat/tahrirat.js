...
const readerHierarchy = {
    ...
    abuAmr: { name: "البصري", rawis: ["دوري أبو عمرو", "السوسي"] },
...
function buildReaderFilter() {
    ...
    const mainReaders = ["نافع", "ابن كثير", "البصري", "ابن عامر", "عاصم", "حمزة", "الكسائي", "أبو جعفر", "يعقوب", "العاشر"];
...
function showPopupMobile(rank, reader, withReaders, rule, element) {
    ...
    // حساب ارتفاع البطاقة بشكل ديناميكي
    const cardHeight = Math.min(200, window.innerHeight * 0.4);
    ...
    if (rect.top + cardHeight > windowHeight - 30) {
        top = rect.top + window.scrollY - cardHeight - 15;
    ...
    // إزالة القيود التي تمنع التوسع
    card.style.whiteSpace = 'normal';
    card.style.wordWrap = 'break-word';
    
    // إضافة سهم
    if (shouldShowAbove) {
        card.style.borderBottom = '5px solid var(--primary-gold)';
        card.style.borderTop = 'none';
    } else {
        card.style.borderTop = '5px solid var(--primary-gold)';
        card.style.borderBottom = 'none';
    }
    
    // ===== إجبار عناصر البطاقة على التوسع =====
    const allValues = card.querySelectorAll('.popup-value');
...
function applyFilters() {
    ...
    const selectedReader = readerSelect ? readerSelect.value : 'all';
    const audioPlayer = document.getElementById('audioPlayer');
    const audioToggleBtn = document.getElementById('toggleAudioPlayerBtn');
    const readerMessage = document.getElementById('readerMessage');

    // الشرط 1: إخفاء مشغل الصوت وزر التحكم إذا كان القارئ منفرداً
    if (selectedReader === 'all') {
        // إظهار مشغل الصوت وزر التحكم
        if (audioPlayer) audioPlayer.style.display = 'flex';
        if (audioToggleBtn) audioToggleBtn.style.display = 'flex';
    } else {
        // إخفاء مشغل الصوت وزر التحكم
        if (audioPlayer) audioPlayer.style.display = 'none';
        if (audioToggleBtn) audioToggleBtn.style.display = 'none';
    }

    // الشرط 2: إظهار رسالة إذا كان القارئ منفرداً
    if (readerMessage) {
        if (selectedReader === 'all') {
            readerMessage.style.display = 'none';
        } else {
            readerMessage.style.display = 'block';
            readerMessage.textContent = '⚠️ الترتيب خاص للقراء العشرة مجتمعين';
        }
    }
}
