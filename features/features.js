...
window.loadPdfPage = function(page) {
    ...
    const pdfPath = `../mushaf-pdfs/${currentMushafFile}.pdf#toolbar=0&navpanes=0&page=${page}`;
...
function bindMobileEvents() {
    ...
    if (mobilePrev) mobilePrev.onclick = () => window.prevPage();
    if (mobileNext) mobileNext.onclick = () => window.nextPage();
    document.addEventListener('click', (e) => {
        if (!e.target.closest('.mobile-dropdown-btn')) closeMobileDropdowns();
    });
}
