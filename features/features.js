...
const mushafsList = [
    ...
    { name: "رواية ورش (طريق الأصبهاني)", file: "mushaf_asbahani.js" },
...
function init() {
    ...
    // ===== إصلاح ظهور الشريط السفلي على الهاتف =====
    setTimeout(function() {
...
// ==================== إصلاح ظهور الشريط السفلي عند تغيير الحجم ====================
function fixMobileBarVisibility() {
    const isMobile = window.innerWidth <= 768;
    const bottomBar = document.querySelector('.mobile-bottom-bar');
...
// الاستماع لتغيير حجم الشاشة (بما في ذلك Zoom)
let resizeTimer;
window.addEventListener('resize', function() {
    clearTimeout(resizeTimer);
    resizeTimer = setTimeout(fixMobileBarVisibility, 200);
});

// الاستماع لتدوير الشاشة
window.addEventListener('orientationchange', function() {
    setTimeout(fixMobileBarVisibility, 300);
});