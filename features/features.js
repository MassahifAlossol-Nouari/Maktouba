...
window.loadPdfPage = function(page) {
    ...
    // مسار الملف مع إعدادات العرض
    const pdfPath = `./features-pdfs/${currentMushafFile}#toolbar=0&navpanes=0&page=${currentPage}`;
    
    // إظهار الـ embed وإخفاء الرسالة البديلة
    ...
    setTimeout(() => {
        ...
        // التحقق من نجاح التحميل بعد 5 ثوانٍ
        setTimeout(() => {
            try {
                // إذا كان embed فارغاً أو صغيراً جداً، نظهر الرسالة البديلة
                if (embed.offsetHeight < 50) {
                    embed.style.display = 'none';
                    if (fallback) {
                        fallback.style.display = 'block';
                        fallback.innerHTML = `
                            ⚠️ لا يمكن عرض PDF مباشرة.
                            <br><br>
                            <a href="${pdfPath}" target="_blank" style="color:var(--primary-gold); text-decoration:underline; font-size:1.1rem;">
                                📄 اضغط هنا لفتح الملف في نافذة جديدة
                            </a>
                            <br>
                            <small style="color:var(--text-light);">(سيفتح في علامة تبويب جديدة دون مغادرة الموقع)</small>
                        `;
                    }
                }
            } catch(e) {
                console.log('⚠️ فشل التحقق من التحميل');
            }
        }, 5000);
